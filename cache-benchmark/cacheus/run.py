...
class AlgorithmTest:
    ...
    def run_all_workload(self, alg, progress_bar):
        ...
        last_total_hit = 0
        last_perfect_hit = 0
        traceInterval = 458 # avazu
        # traceInterval = 916 # criteo-kaggle
        # traceInterval = 916 # criteo-terabyte
        ...
        for lba, write, _ in self.reader.read():
            ...
            if perfect_hit_iterator == 0:
                ...
                request_number += 1
                if request_number % traceInterval == 0:
                    with open('/home/xxx/cache-benchmark/cacheus.log','a') as file: 
                        file.write('%f\n' % (max(1, (self.perfect_hit - last_perfect_hit)) / traceInterval))
                    last_perfect_hit = self.perfect_hit
                    
                    # output total hit rate
                    # ios = self.reader.num_requests()
                    # hits = ios - self.misses
                    # with open('/home/xxx/cache-benchmark/cacheus.log','a') as file: 
                    #     file.write('%f\n' % ((hits - last_total_hit) / traceInterval / 26))
                    # last_total_hit = hits
