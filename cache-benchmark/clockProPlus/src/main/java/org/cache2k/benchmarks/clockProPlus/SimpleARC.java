...
public class SimpleARC implements ISimpleCache {
    ...
    public boolean request(String address) {
        ...
        if (this.topLRU1.size() + this.bottomLRU1.size() == this.size) {
            if (this.topLRU1.size() < this.size) {
                String firstKey = this.bottomLRU1.firstKey();
                this.bottomLRU1.remove(firstKey);
                this.replace(address);
            } else {
                String firstKey = this.topLRU1.firstKey();
                this.topLRU1.remove(firstKey);
            }
        } else {
            if (this.topLRU1.size() + this.bottomLRU1.size() + this.topLRU2.size()
                    + this.bottomLRU2.size() >= this.size) {
                if (this.topLRU1.size() + this.bottomLRU1.size() + this.topLRU2.size() + this.bottomLRU2.size() == 2
                        * this.size) {
                    String firstKey = this.bottomLRU2.firstKey();
    ...
    public String toString() {
        StringBuffer str = new StringBuffer(String.format("ARC(%d)", this.size));
        str.append(this.topLRU1.asList().toString());
        str.append(this.topLRU2.asList().toString());
        str.append(this.bottomLRU1.asList().toString());
        str.append(this.bottomLRU2.asList().toString());
        return str.toString();
    }

    protected void replace(String key) {
        if (!this.topLRU1.isEmpty() && (this.topLRU1.size() > this.recencySize
                || (this.bottomLRU2.containsKey(key) && this.topLRU1.size() == this.recencySize))) {
            String firstKey = this.topLRU1.firstKey();
            CacheMetaData tmpData = this.topLRU1.get(firstKey);
            this.topLRU1.remove(firstKey);
            this.bottomLRU1.put(firstKey, tmpData);
        } else {
            String firstKey = this.topLRU2.firstKey();
    ...
    protected void printSize() {
        if (this.topLRU1.size() + this.topLRU2.size() > this.size) {
            System.err.printf("%d, %d, %d, %d, %d\n", this.topLRU1.size(), this.topLRU2.size(), this.bottomLRU1.size(),
                    this.bottomLRU2.size(), this.recencySize);
        }
    }
    ...
    protected LinkedMap<String, CacheMetaData> topLRU1;
    protected LinkedMap<String, CacheMetaData> topLRU2;
    protected LinkedMap<String, CacheMetaData> bottomLRU1;
    protected LinkedMap<String, CacheMetaData> bottomLRU2;
