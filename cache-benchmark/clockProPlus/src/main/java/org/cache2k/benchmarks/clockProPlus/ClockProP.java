 ...
 *
 * <p>
 * The metrics reshuffleCount and scanCount were added for further analysis and
 * comparison of the works efforts by this implementation by Jens Wilke.
 *
 */
@SuppressWarnings({ "Duplicates", "WeakerAccess", "RedundantCast", "UnusedAssignment", "unused", "Convert2Diamond",
        "UnnecessaryLocalVariable" })
public class ClockProP implements ISimpleCache {

    public ClockProP(int size, ClockPro.Tuning tuning) {
        this.size = size;
        this.clockMap = new HashedMap<String, CacheMetaData>();
    ...
    public String getStatisticsString() {
        ...
        String result = String.format(
                "avg. %f, hot hit %d, cold hit %d, cold access %d, demote hit %d, reshuffles %d, scans %d", avgHotRatio,
                this.hotHit, this.coldHit, this.testAccess, this.demoteHit, this.reshuffleCount, this.scanCount);
        return result;
    }
    ...
    protected void addToClock(CacheMetaData data) {
        ...
        data.setInStackStatus(true);
        this.clockMap.put((String) data.getAddress(), data);
    }
