#include "VineCache_MFU_Cython.hpp"
...
set<Cache_data> level_sets[N_LEVELS];
unordered_map<string, set<Cache_data>::iterator> experience_map;
...
/**
 * @brief Evict an entry from the cache using a random walk MFU strategy.
 */
void evict_entry() {
    recent_evictions++;
    ...
    for (int i = 0; i < N_LEVELS; i++) {
        if (level_sets[i].empty()) continue;
        
        int ts = level_sets[i].begin()->last_access_timestamp;
        if (ts < earliest_ts) {
            earliest_ts = ts;
            target_level = i;
        }
    ...
    if (target_level != -1) {
        auto it = level_sets[target_level].begin();
        string key_to_evict = it->key;
        experience_map.erase(key_to_evict);
        level_sets[target_level].erase(it);
...
void cache_insert(const string& key, vector<float>* val_ptr, double exp) {
    ...
    // Initial insertion with count 1
    auto res = level_sets[level].insert(Cache_data(key, val_ptr, exp, 1, global_clock));
    experience_map[key] = res.first;
...
/**
 * @brief Handle a cache hit by updating experience, frequency count, and level.
 */
void cache_hit(const string& key, double exp_delta) {
    ...
    // Remove old state and insert new state
    level_sets[old_level].erase(it->second);
    auto res = level_sets[new_level].insert(Cache_data(key, val_ptr, new_exp, new_count, global_clock));
    experience_map[key] = res.first;
...
/**
 * @brief Generate a unique cache key from table and row identifiers.
 */
inline string generate_cache_key(int table_id, int row_id) {
 ...
 */
void request_to_vinecache(vector<int>& group_keys, int& group_hit_total, int& group_perfect_hit, 
