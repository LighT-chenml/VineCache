...
unordered_map<string, set<Cache_data>::iterator> cache_index;
...
void evict_entry() {
    ...
    for (int level = 0; level < N_LEVELS; level++) {
        if (level_sets[level].empty()) continue;
        
        int timestamp = level_sets[level].begin()->last_access_timestamp;
    ...
    if (target_level != -1) {
        auto iter = level_sets[target_level].begin();
        string evicted_key = iter->key;
        cache_index.erase(evicted_key);
        level_sets[target_level].erase(iter);
        current_size--;
...
void cache_insert(const string& key, vector<float>* value, double experience) {
    ...
    auto iter = level_sets[level].insert(Cache_data(key, value, experience, 1, timestamp)).first;
    cache_index[key] = iter;
    current_size++;
...
void cache_hit(const string& key, double experience) {
    ...
    // Move to new level with updated values
    cache_index[key] = level_sets[new_level].insert(
        Cache_data(key, iter->second->embedding_value, new_experience, new_count, timestamp)).first;
    level_sets[old_level].erase(iter->second);
